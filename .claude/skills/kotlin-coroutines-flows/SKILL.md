...
```kotlin
class ItemListViewModel @Injeect constructor() : FlowMviViewModel<ItemState, ItemEvent>(
    ...
    private fun deleteItem(id: String) {
        viewModelScope.launch {
            ...
            internalEffects.emit(Effect.ShowSnackbar("Item deleted"))
        }
    }
}
...
viewModelScope.launch {
    try {
        ...
        internalState.update { it.copy(data = data) }
    } finally {
        internalState.update { it.copy(isLoading = false) }  // always runs, even on cancellation
    }
}
```
...
internal class FakeItemRepository : ItemRepository {
    ...
    fun emit(items: List<Item>) { this.items.value = items }

    override suspend fun getItemsByCategory(category: String): Result<List<Item>> {
        return Result.success(items.value.filter { it.category == category })
    }
}
```
